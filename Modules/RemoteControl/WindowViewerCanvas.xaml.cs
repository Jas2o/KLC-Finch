...
namespace KLC_Finch {
    ...
    /// </summary>
    public partial class WindowViewerCanvas : WindowViewer {
        ...
        public WindowViewerCanvas(IRemoteControl rc, int virtualWidth = 1920, int virtualHeight = 1080, Agent.OSProfile endpointOS = Agent.OSProfile.Other, string endpointLastUser = "") {
            InitializeComponent();
        ...
        private void LoadSettings(bool isStart = false) {
            if (isStart) {
                ...
                if (useMultiScreen) {
                    /*
                    if (Settings.Startstate.controlEnabled)
                ...
                //SetControlEnabled(Settings.Startstate.controlEnabled, true);
            }
        ...
        private void HandleCanvasMouseMove(object sender, MouseEventArgs e) {
            ...
            if (useMultiScreen) {
                ...
                if (!state.controlEnabled || !this.IsActive)
                    return;
            ...
            /*
            if (!state.controlEnabled || connectionStatus != ConnectionStatus.Connected)
                return;
            ...
            if (useMultiScreen) {
                for (int i = 0; i < ListScreen.Count; i++) {
                    ...
                    if (ListScreen[i].Texture != null) {
                        Color multiplyColor;
                        if (ListScreen[i] == state.CurrentScreen)
                            multiplyColor = Color.White;
                        else if (state.controlEnabled || virtualViewWant == virtualCanvas) //In overview, or it's on the edge of focused screen
        ...
        private void Window_StateChanged(object sender, EventArgs e) {
            if (WindowState == System.Windows.WindowState.Minimized) {
                ...
                if (glSupported && state.powerSaving) {
